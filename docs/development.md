...
- **Offline**: `workbox.navigateFallback: "/index.html"` is set explicitly — without it, `generateSW`'s default precache doesn't serve `index.html` for arbitrary client-side routes, so an offline deep-link/reload to e.g. `/play/add` would fail even though the shell is cached. `globPatterns` is widened beyond the workbox default to include `woff2` (Fredoka variable font files) and `mp3` (Howler sound effects) so the app is fully playable, sound and all, with no network.
